...
У контроллера могут быть перехвачены некоторые типы событий, которые можно
использовать для более гибкой работы с действиями. У нас имеются следующие
событие, которые можно перехватить:
...
В методах before, after и exception можно использовать `render` различного рода контента.
В них можно передавать использовать весь тот же функционал, который используется в обычном
методе контроллера.
...
В этом примере любое действие контроллера прерветься ошибкой 404 notfound, если мы не
...
еще более гибкую систему контроллеров. Для примера можно создать контроллер `Administration`,
и прописать в хуке `onBefore` проверку прав доступа, далее все контроллеры админ-панели
наследовать от этого класса.

Не забывайте вызывать в хуке методы класса родителя, для примера:
